...
class BaseGameLogic(GamePartLogicTemplate):
    ...
    def play(self, **kwargs):
        ...
        if GV.WAYS:
            ...
            self.send_payouts(self.evaluate_lines(),"BaseGame")
    ...
    def spin_reels(self,bonus):
        ...
        weightset = self.weightsets[bonus][random_base]
        reelset = self.reelsets[bonus][random_base]
        ...
        for coin_place in self.coin_places:
            self.current_coinpicture[coin_place] = self.coins_has.draw_random()
        ...
        info_dict = {
            ...
            "current_coinpicture": self.current_coinpicture,
            "reelset": reelset,
            "reelset_enum": self.data["reelsets_enum"][bonus][random_base],
    ...
    def evaluate_lines(self):
        line_infos = []
        paylines = self.data.get("paylines", [])
        paytable = self.data.get("paytable")
        for line in paylines:
            leading_joks = 0
            match_len = 0
            symbol = None
            for reel_idx in range(len(self.reelpicture)):
                s = self.reelpicture[reel_idx][line[reel_idx]]
                if s in self.wilds:
                    if symbol is None:
                        leading_joks += 1
                    else:
            ...
            if match_len < 2:
                continue
            jok_win = paytable.get_win(("_JOK_1_", leading_joks))
            sym_win = paytable.get_win((symbol, match_len)) if symbol is not None else 0
            if jok_win > sym_win:
                single_win = jok_win
                length = leading_joks
            else:
            ...
            if not single_win:
                continue
            credit_win = single_win * self.selected_bet_multiplier
            info_dict = {
                "sound": "line_win",
                "line_win_coordinates": line,
                "length": length,
                "reelpicture": self.reelpicture_enum,
            }
            line_infos.append([credit_win, info_dict])
        return line_infos
    ...
    def evaluate_ways(self):
        ...
        for sym in self.paytable.symbols:
            ...
            for reel_idx, pict in enumerate(self.reelpicture):
                pict = list(pict)
                if sym in pict or "_JOK_1_" in pict:
                    mult *= (pict.count("_JOK_1_") + pict.count(sym))
                    length+=1
    ...
    def check_triggers(self, coin_trigger_count, sca_trigger_count, bn_trigger_count,
        ...
        if len(self.coin_places) >= coin_trigger_count:
            ...
            pp_trigger = 0 == self.data["pp_trigger"][len(self.coin_places)-1].draw_random()
        ...
        if len(self.sca_places) >= sca_trigger_count:
            if mode=='fg':
                self.meters[MC.FREE_GAMES].increase_value(5)
                self.send_wininfo("GAME_SPECIFIC", "retrigger_fg")
            else:
                self.meters[MC.FREE_GAMES].set_value(5)
